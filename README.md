...
<b>Восстановление Состояния:</b> Одной из ключевых особенностей стейт-машины является способность "запоминать" текущее
состояние диалога с пользователем. Даже после долгого перерыва, бот может продолжить общение с той точки, на которой
оно было прервано.

<b>Изолированные стейты:</b> Стейт-машина фреймворка позволяет изолировать отдельные стейты друг от друга и
обеспечивает слабую связанность кода. Тем самым исключается необходимость в глобальном контексте для хранения
информации о текущей сессии.

### Аналогия:
Для наглядности можно сравнить стейт-машину фреймворка с браузером: каждый стейт представляет собой отдельную
веб-страницу, позволяя легко управлять диалогом.
...
Предполагается, что вы владеете либо имеете достаточное представление по работе с Django, Poetry, Pydantic
...
version = "0.1.0"
description = ""
authors = ["YourNmae <your_mail@mail.ru>"]
readme = "README.md"
...
Переместите содержимое папки проекта `django_tg_bot` в папку проекта `django_project`, предварительно переименовав родительскую папку django_tg_bot.
...
### По завершению настройки структура дерево проекта должно выглядеть следующим образом:
...
└── tests
    └── __init__.py
```
### Теперь можно запуcтить бота на локальном сервере:
`./manage.py runserver`
